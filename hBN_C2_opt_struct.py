...
i = 5
with open('screened_dzv_opt_struct_{}.txt'.format(i), 'w') as scr:
    with open('unscreened_dzv_opt_struct_{}.txt'.format(i), 'w') as uscr:
        nx = i
        ny = i
        # alat = 2.50
        # clat = 20.0 
        ...
        mf.xc = 'pbe'
        mf.with_df._cderi_to_save = 'hbn_c2_pbc_gdf_{}x{}x1_dzv_opt.h5'.format(nx,ny)
        mf.chkfile = 'hbn_c2_gdf_{}x{}x1_dzv_opt.chk'.format(nx,ny)
#        dm = mf.from_chk('hbn_c2_gdf_{}x{}x1_dzv_opt.chk'.format(nx,ny))
...
#        mf.kernel(dm)
        ...
        lib.chkfile.dump('hbn_c2_{}x{}x1_dzv_opt.chk'.format(nx,ny), 'C_loc', C_loc)
        
        C_loc = lib.chkfile.load('hbn_c2_{}x{}x1_dzv_opt.chk'.format(nx,ny), 'C_loc')
        
        from pyscf.tools import molden
        with open('hbn_c2_gdf_{}x{}x1.molden'.format(nx,ny), 'w') as f1:
            molden.header(cell, f1)
            molden.orbital_coeff(cell, f1, C_loc)

        mycRPA = cRPA(mf, 'hbn_c2_pbc_gdf_{}x{}x1_dzv_opt.h5'.format(nx,ny), loc_coeff = C_loc)
        ...
        scr.write('t = {}'.format(mycas.get_h1eff()))
